...
async fn start_command_process(
    ...
    let (output_file, stderr_file) = create_output_file(&session.output_file_path)
        .map_err(|message| CommandFailure::Start { message })?;
    ...
    let child = command.spawn().map_err(|error| CommandFailure::Start {
...
async fn run_foreground_command(
    ...
    let tail_task = tokio::spawn(async move {
        tail_file(output_file_path, false, tx_clone, done_rx.clone()).await;
    });
