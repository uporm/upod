...
use crate::{
    ...
    models::{
        FileInfo, FileMetadata, FileNode, Permission, RenameFileItem, ReplaceFileContentItem,
    },
...
impl SandboxHandle {
        ...
        if status.is_success() {
            ...
            match serde_json::from_str::<HashMap<String, FileInfo>>(&body) {
                ...
                Err(e) => Err(UpodError::Client(format!(
                    "Failed to parse FileInfo map: {} from body: {}",
                    e, body
                ))),
            }
        } else {
    ...
    pub async fn search_files(&self, path: &str, pattern: Option<&str>) -> Result<Vec<FileInfo>> {
        ...
        if status.is_success() {
            ...
            match serde_json::from_str::<Vec<FileInfo>>(&body) {
                Ok(data) => Ok(data),
                Err(e) => Err(UpodError::Client(format!(
                    "Failed to parse FileInfo list: {} from body: {}",
                    e, body
                ))),
            }
        } else {
    ...
    /// 获取文件列表（支持树形递归）
    /// 
    /// GET /files?path=...&sortBy=...
    pub async fn list_files(&self, path: &str, sort_by: Option<&str>) -> Result<Vec<FileNode>> {
        let bridge_url = self.get_bridge_url(44321);
        let url = format!("{}/files", bridge_url);
        ...
        if status.is_success() {
            let body = response.text().await?;
            match serde_json::from_str::<Vec<FileNode>>(&body) {
                Ok(data) => Ok(data),
                Err(e) => Err(UpodError::Client(format!(
                    "Failed to parse FileNode list: {} from body: {}",
                    e, body
                ))),
            }
        } else {
    ...
    pub async fn replace_content(
        &self,
        items: &HashMap<String, ReplaceFileContentItem>,
    ) -> Result<()> {
        let bridge_url = self.get_bridge_url(44321);
